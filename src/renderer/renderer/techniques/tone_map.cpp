    ...
    , m_luminance_target(physical_luminance_to_reference_luminance(m_peak_intensity))
{
    if (!m_is_hdr)
    {
        m_luminance_target = physical_luminance_to_reference_luminance(SDR_PAPER_WHITE);
    ...
    m_tone_map_parameters_buffer = m_render_resource_blackboard.create_buffer(TONE_MAP_PARAMETERS_BUFFER_NAME, tone_map_buffer_create_info);
