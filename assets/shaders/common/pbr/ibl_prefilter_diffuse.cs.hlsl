...
float3 prefilter_irradiance(float3 R)
{
    ...
    {
        float3 L = tangent_to_world(ren::sampling::hemisphere_cosine_weighted(i, pc.samples), N);
        float NdotL = saturate(dot(N, L));

        // Using lower mip level to forcibly reduce frequency.
        float3 cube_sample = rhi::uni::tex_sample_level_cube<float4>(pc.source_cubemap, REN_SAMPLER_LINEAR_WRAP, L, 0.0).xyz;
